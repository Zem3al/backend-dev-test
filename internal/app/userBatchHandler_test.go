...
func BenchmarkUploadData(b *testing.B) {
	b.Run("Endpoint: POST /user/batch", func(b *testing.B) {
		r, _ := http.NewRequest("POST", "/user/batch", strings.NewReader(testData))
		w := httptest.NewRecorder()

		err := persistence.LoadPayloadRespositoryMock(context.Background())
		if err != nil {
			log.Fatal(err)
		}
		...
		handler := http.HandlerFunc(UploadData)
